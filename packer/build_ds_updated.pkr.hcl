...
build {
  ...
  #dormant friendy patching file config
  provisioner "file" {
    source      = "./scripts/linux/dormant_auto_off.sh"
 ...
 ###### install docker engine: instructions for ubuntu. from https://docs.docker.com/engine/install/ubuntu/
  ...
  provisioner "shell" {
    script        = "./scripts/linux/update_and_upgrade.sh"
    remote_folder = "."
  }

  # Add Docker's official GPG key:
  provisioner "shell" {
    script        = "./scripts/linux/official_docker_key_add.sh"
    remote_folder = "."
  }
  ...
  provisioner "shell" {
    script        = "./scripts/linux/add_repo_to_apt.sh"
    remote_folder = "."
  }
  ...
  provisioner "shell" {
    script        = "./scripts/linux/install_docker_latest.sh"
    remote_folder = "."
  }
  ...
  provisioner "file" {
    source      = "./scripts/linux/daemon.json"
