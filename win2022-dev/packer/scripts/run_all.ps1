...
.NOTES
    ...
    FIX: Replaced all smart/curly quotes with straight quotes to prevent

         PowerShell parse errors (TerminatorExpectedAtEndOfString).

    FIX: $psSuccess now derived from $LASTEXITCODE instead of $? to avoid

         false positives from cmdlets inside called scripts.
...
& $ScriptPath
    ...
    # FIX: Capture exit code from $LASTEXITCODE only.

    # Using $? here is unreliable — it reflects the last native command inside

    # the called script, not the script's overall success. Derive success purely

    # from $LASTEXITCODE to avoid false positives.

    $exitCode  = $LASTEXITCODE

    $psSuccess = ($exitCode -eq 0) -or ($null -eq $exitCode)
 
    if (-not $psSuccess -and -not $AllowFailure) {

        Write-Error "$Label FAILED with exit code $exitCode -- aborting hardening pipeline"

        exit $exitCode

    } elseif (-not $psSuccess -and $AllowFailure) {
