...
.NOTES
    ...
    6. apply_audit_policy.ps1 runs AFTER apply_mof.ps1 (V-278942 to V-278947)
    ...
    FIX: Improved Invoke-Step to reliably detect non-zero exit codes and prevent
         phantom exit codes (like from DISA downloads) from failing the build.
    ...
    FIX: Added apply_audit_policy.ps1 (V-278942 to V-278947) and
         apply_remaining_fixes.ps1 (V-254251/258/261) to Step 4.
#>
...
$integrityChecks = @(
    ...
    @{ Path = "$scriptDir\audit.ps1";                      MinLines = 30;  Label = "apply_audit_policy.ps1"     }
...
# Must run BEFORE create_mof.ps1 so DSC certificate checks find certs already installed.
# -----------------------------------------------------------------------
